...
Đồ án trình bày pipeline tái lập cho bài toán phân loại 36 lớp ảnh bàn tay ASL-HG. Raw image canonical được crop ROI bởi MediaPipe Hand Landmarker với padding 0,10, sau đó MobileNetV4 Conv-S ImageNet được fine-tune toàn bộ. Mô hình \texttt{mp-mnv4-003-p010} đạt Accuracy 96,38\% và Macro-F1 95,64\% trên test P9; validation/test có detection coverage 100\%. Khi không detect được tay, pipeline dùng raw image fallback và ghi manifest. Các số liệu chỉ áp dụng cho participant split đã khóa.
...
Cấu hình, crop manifest, split, checkpoint, metrics và environment của run được chọn nằm trong artifact bất biến \texttt{mp-mnv4-003-p010}, commit \texttt{497a5ac4fead1c52c868e6f92e84002db5210c1c}. Cấu hình phần cứng chưa được tổng hợp để kết luận về tốc độ.
