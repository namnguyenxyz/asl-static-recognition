...
MediaPipe Hand Landmarker được dùng để phát hiện bàn tay và suy ra 21 điểm mốc trên bàn tay \cite{mediapipe}. Theo thiết kế của MediaPipe, bộ phát hiện lòng bàn tay xác định vùng bàn tay, sau đó mô hình landmark xử lý vùng crop. Trong run \texttt{mp-svm-001}, đặc trưng landmark hai tay có 130 chiều được chuẩn hóa bằng StandardScaler và phân loại bằng RBF SVM. Với MobileNetV4, các run hiện có dùng archive ảnh đã xử lý của dataset, đổi kích thước theo data configuration của mô hình và chuẩn hóa bằng mean/std ImageNet.

Định vị ROI từ landmark và đưa ROI vào MobileNetV4 là hướng cải tiến dự kiến. Khi thực hiện, phương pháp chỉ được gọi là phát hiện/định vị và trích xuất ROI dựa trên MediaPipe; không gọi là phân đoạn theo từng điểm ảnh nếu không có segmentation mask riêng.
...
Các chỉ số gồm Accuracy, Precision, Recall và F1-score theo lớp. Confusion matrix được sử dụng để quan sát các cặp lớp dễ nhầm, đặc biệt là hai hướng O$\rightarrow$0 và 0$\rightarrow$O. Với nguyên mẫu thời gian thực, nhóm ghi nhận độ trễ cho bước định vị bàn tay và bước suy luận mô hình, cùng FPS; các chỉ số này không được thay thế bằng nhận định định tính như ``nhạy'' hoặc ``ổn định''.
