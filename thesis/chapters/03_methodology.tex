...
Kiểm kê kiểm tra số lớp, khả năng đọc ảnh, hash và ảnh trùng. Các run dùng một canonical representative cho mỗi SHA-256 để loại leakage. Run \texttt{mp-mnv4-003-p010} lấy raw image canonical theo manifest/split khóa, dùng MediaPipe Hand Landmarker một tay tạo ROI vuông padding 0,10, rồi dùng raw image fallback nếu detection/ROI thất bại. Crop manifest, coverage, checkpoint và metrics được publish cùng artifact.
...
\begin{table}[ht]
\centering
\caption{Cấu hình huấn luyện của run \texttt{mp-mnv4-003-p010}}
...
CNN, MobileNetV4 frozen, MediaPipe landmarks+SVM, \texttt{mnv4-002} và \texttt{mp-mnv4-003-p010} dùng cùng canonicalization và participant split. \texttt{mp-mnv4-003-p010} fine-tune toàn bộ backbone với learning rate $10^{-4}$; policy fallback được khóa trước khi đọc test. So sánh \texttt{mnv4-002} (archive processed) với \texttt{mp-mnv4-003-p010} (raw image--MediaPipe ROI) là ablation đầu vào hiện có; one-hand/two-hand và external evaluation còn là hướng mở rộng.
