...
\begin{table}[ht]
\centering
\caption{Cấu hình huấn luyện baseline dự kiến}
...
Các baseline CNN, MobileNetV4 frozen, MobileNetV4 full fine-tuning và MediaPipe landmarks+SVM dùng cùng canonicalization và participant split. Fine-tuning hiện có mở toàn bộ pretrained backbone với learning rate $10^{-4}$; đây là một lựa chọn thực nghiệm cụ thể, không phải quy tắc chung cho mọi kiến trúc. Hướng cải tiến MediaPipe-guided crop/normalization kết hợp MobileNetV4 cần được so sánh với \texttt{mnv4-002} bằng cùng split, policy fallback được khóa trước khi xem metrics test, và ablation raw image/crop, one-hand/two-hand, landmark-only/image-only. Tập kiểm thử không được dùng để chọn các phương án này.
